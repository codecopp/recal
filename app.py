# app.py
# ---------------------------------------------
# Streamlit: 2탭 계산기 (성과보수 중심 / 청구액 중심)
# - 각 탭 독립 세션키 사용
# - 억/천만 가로 입력
# - 프리셋 버튼 + 수동 비율 입력
# - 결과는 카드형 지표
# - 청구액 중심 탭: 성과보수 상한 5억 적용
# ---------------------------------------------
import streamlit as st

st.set_page_config(page_title="성과보수·청구액 계산기", layout="centered")
st.title("💼 성과보수·청구액 계산기")
...
# 탭 구성
# -----------------------------
tab1, tab2 = st.tabs(["성과보수 중심", "청구액 중심"])
...
with tab1:
    ...
    if st.session_state["calculated_1"]:
        if 0 < fee_input <= CAP_FEE and 0 <= st.session_state["ratio_y_1"] < 100:
            ...
            m1, m2 = st.columns(2)
            with m1:
                st.metric("청구액 (x)", f"{x/100_000_000:.1f} 억 원")
            with m2:
...
with tab2:
    ...
    st.caption(f"입력된 청구액: {label_eok_chunman('total_eok','total_chunman')}  (= {total_input/100_000_000:.1f}억 원)")
    ...
    if st.session_state["calculated_2"]:
        if total_input > 0 and 0 <= st.session_state["ratio_y_2"] < 100:
            ...
            st.caption(
                f"입력 기준 · 청구액: {x/100_000_000:.1f}억 · 인용비율: {st.session_state['ratio_y_2']:.1f}% · "
                f"성과보수 상한: {CAP_FEE/100_000_000:.1f}억"
            )
        else:
            st.warning("입력 오류: 청구액은 0 초과, 인용비율은 100% 미만이어야 합니다.")
